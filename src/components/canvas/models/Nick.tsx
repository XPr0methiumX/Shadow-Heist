...
export function Nick(props: JSX.IntrinsicElements['group']) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key.toLowerCase() === 'w') {
        setMovementState(event.shiftKey ? 'running' : 'walking')
      }
    }
  ...
  useEffect(() => {
    ...
    const currentAction = actions[movementState === 'running' ? 'run' : movementState]
    if (currentAction) {
  ...
  useFrame(() => {
    if (group.current) {
      ...
      // cameraTarget.current.y += 2.5; // Adjust this value for camera height

      // // Smoothly move the camera to follow the target
      // camera.position.lerp(cameraTarget.current, 0.1);
      // camera.position.z = group.current.position.z + 5; // Keep the camera 5 units behind the character
